...
export async function buildSite({ pagesDir, distDir = 'dist' }: BuildSiteOptions) {
...
async function getAllRoutes(pagesDirAbs: string, basePath = ''): Promise<RouteInfo[]> {
	...
	for (const item of items) {
		...
		if (itemStat.isDirectory()) {
			...
			if (isDynamic) {
				...
				routes.push({ path: routePath, outputFile: outputPath, isDynamic: false })
			}
		}
	}
...
async function handleDynamicRoute(routePath: string, fileAbsPath: string): Promise<RouteInfo[]> {
	try {
		const module = await import(pathToFileURL(fileAbsPath).href)
		if (!module.generateStaticParams) {
			console.warn(`Dynamic route ${routePath} missing generateStaticParams function`)
			return []
		}

		const staticParams: GenerateStaticParamsResult[] = await module.generateStaticParams()
		...
		return staticParams.map((paramResult) => {
			...
			const outputPath = routePath.replace(`[${paramName}]`, paramValue) + '/index.html'
...
async function generateRoute(route: RouteInfo, pagesDirAbs: string, distDirAbs: string) {
	...
	const pageElement = Page && Page.constructor && Page.constructor.name === 'AsyncFunction' ? (
